...
import { CreditCard, UploadCloud, X, CheckCircle2 } from 'lucide-react';

interface ApronUploadFrameProps {
  value?: string;
  onChange: (base64: string) => void;
  label?: string;
}

export default function ApronUploadFrame({ value, onChange, label = 'Apron Kart Görseli' }: ApronUploadFrameProps) {
    ...
    <div className="w-full space-y-2">
      <div className="flex items-center justify-between">
        <label className="block text-xs font-bold uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
          <CreditCard className="w-4 h-4 text-talpa-gold-600" />
          <span>{label}</span>
          <span className="text-red-500">*</span>
        </label>
        <span className="text-[11px] text-slate-500 font-medium">Apron / Havalimanı Giriş Kartı</span>
      </div>
        ...
        {value ? (
          <div className="relative w-full h-full group">
            ...
            <div className="absolute bottom-2 left-2 bg-emerald-700/90 backdrop-blur-md text-white text-[11px] px-2.5 py-1 rounded-md font-medium flex items-center gap-1.5 shadow">
              ...
              <span>Apron Kartı Yüklendi</span>
            </div>
          </div>
        ) : (
          <div className="text-center space-y-2 p-2">
            <div className="w-12 h-12 rounded-xl bg-white border border-slate-200 shadow-xs text-talpa-navy-800 flex items-center justify-center mx-auto">
              <UploadCloud className="w-6 h-6 text-talpa-navy-700 animate-bounce" />
            </div>
            <div>
              <p className="text-xs font-bold text-slate-800">
                Apron Kart görselinizi yüklemek için tıklayın veya sürükleyin
              </p>
              <p className="text-[11px] text-slate-500 mt-0.5">
                Sabiha Gökçen Havalimanı (SAW) Apron Giriş Kartınızın ön yüzü.
              </p>
            </div>
          </div>
        )}
